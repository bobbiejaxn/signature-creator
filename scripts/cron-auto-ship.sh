#!/usr/bin/env bash
# cron-auto-ship.sh
# Fetches ONE issue labeled spec-ready (auto-approved), runs the full /ship workflow
...
check_dlq_retry_limit() {
  ...
  if [ "$dlq_count" -ge "$DLQ_MAX_RETRIES" ]; then
    ...
    "$GH_BIN" issue edit "$issue_num" --repo "$REPO" --remove-label "spec-ready" --add-label "spec-hold" 2>/dev/null || true
...
# ── Loop: ship all spec-ready issues in queue ───────────────────────────────
SHIPPED_COUNT=0
...
while true; do
  log "Fetching next issue labeled spec-ready (auto-approved)..."
  ...
  ISSUE_NUMBER=$("$GH_BIN" issue list \
    ...
    --jq '[.[] | select(
      ...
      (.labels | map(.name) | contains(["blocker"]) | not)
    ) | .number] | first // empty' 2>/dev/null)

  if [ -z "$ISSUE_NUMBER" ]; then
    log "No more spec-ready issues in queue. Done. (Shipped: $SHIPPED_COUNT)"
    break
  fi

  # Check if this issue was already skipped (DLQ limit, label change failed, etc.)
  if echo " $SKIPPED_ISSUES " | grep -q " $ISSUE_NUMBER "; then
    log "Issue #$ISSUE_NUMBER was already skipped in this run but still has spec-approved. Label change may have failed (archived repo?). Stopping loop."
    break
  fi
  ...
  # Ping the Ollama Cloud proxy to confirm the model endpoint is reachable.
  # If unreachable after 3 retries, try fallback model before giving up.
  check_model_health() {
    local model="$1"
    local max_retries=3
    local retry=0
    while [ $retry -lt $max_retries ]; do
      ...
      http_code=$(curl -sS -o /dev/null -w '%{http_code}' --connect-timeout 5 --max-time 15 \
        http://localhost:9099/v1/models 2>/dev/null || echo "000")
      if [ "$http_code" = "200" ]; then
        log "[HEALTH] Model proxy reachable (HTTP $http_code) — model: $model"
        return 0
      fi
      retry=$((retry + 1))
      log "[HEALTH] Model proxy unreachable (HTTP $http_code), retry ${retry}/${max_retries}..."
      sleep 5
    done
    log "[HEALTH] Model proxy failed all ${max_retries} checks"
    return 1
  }
...
" 2>/dev/null || echo "parse error")
  ...
  FALLBACK_MODEL="${CRON_SHIP_MODEL_SECONDARY:-minimax-m2.7:cloud}"
  FALLBACK_PROVIDER="${CRON_SHIP_PROVIDER_SECONDARY:-ollama}"
...
${GH_BIN} issue edit ${ISSUE_NUMBER} --repo ${REPO} --add-label shipped --remove-label in-progress --remove-label spec-ready
Do NOT close the issue — the PR will auto-close it on merge.
